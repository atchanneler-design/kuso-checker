...
- 筆者自身の有料note・Brain・コンサル等の情報商材への高額な金銭誘導があれば60以上
- 【重要】GoogleやOpenAIなどの正規SaaS・公式ツールの料金紹介（例：月額数万円の公式プラン等）は「情報商材への誘導」には含めず、無害として扱うこと。あくまで「筆者が独自に販売・誘導している怪しい商材」のみを有害とみなす。
...
返却JSON（totalは含めない）:
{
  ...
  "price_warning": "値段への言及があれば記載、なければnull。ただし公式SaaS等ツールの料金紹介（月額◯円等）は対象外とし、筆者自身の情報商材やコンサルの販売価格のみを警告対象とする",
